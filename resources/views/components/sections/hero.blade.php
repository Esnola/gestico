...
<section class="relative min-h-dvh overflow-hidden bg-brand-mist dark:bg-brand-night flex flex-col items-center justify-center p-10 ">
    <div class="grid lg:flex gap-x-12 lg:items-center justify-center {{ $sectionClass }}">
        <div class="max-w-4xl ">
                    ...
                    <div class="grid grid-cols-2 gap-3 py-12 items-center justify-center ">
                        @foreach ($badges as $badge)
                            <span class="inline-flex items-center gap-2 rounded-md border border-brand-border-warm bg-white px-4 py-2 text-sm text-brand-text-strong dark:border-brand-night-border dark:bg-brand-night-raised dark:text-brand-night-text">
                                @if ($badge['icon'])
                                    <x-service-icon :type="$badge['icon']" class="size-4 shrink-0" data-hero-badge-icon="true"/>
                                @endif
    ...
    @if (filled($datos))
        <div class="mx-auto grid gap-4 sm:grid-cols-3 lg:mt-4 sm:max-w-fit">
