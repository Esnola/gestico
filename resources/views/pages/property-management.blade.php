<x-layouts.public title="Administración de fincas">
    <section class="bg-[#213639] py-16 text-white sm:py-20">
        <div class="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
            <p class="text-sm font-bold uppercase text-[#d4a824]">Administración de fincas</p>
            <h1 class="mt-4 max-w-4xl text-4xl font-black leading-tight sm:text-5xl">Acompañamos a presidentes y juntas con gestión diaria, criterio y documentación preparada.</h1>
    ...
    <section class="bg-[#f7f4ed] py-16">
        <div class="mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:px-8">
            <div>
                <h2 class="text-3xl font-black text-[#1f3336]">Gestión ordinaria y extraordinaria</h2>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                @foreach (['Juntas y actas', 'Mantenimiento', 'Seguros y siniestros', 'Morosidad', 'Contratos', 'Obras y derramas'] as $item)
                    <div class="rounded-md border border-[#d8d0bd] bg-white p-5 text-lg font-black text-[#294044]">{{ $item }}</div>
                @endforeach
            </div>
        </div>
    </section>
