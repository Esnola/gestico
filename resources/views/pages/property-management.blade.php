<x-layouts.public title="Administración de fincas">
    <section class="bg-brand-ink-panel py-16 text-white sm:py-20">
        <div class="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
            <p class="text-sm font-normal uppercase tracking-[0.04em] text-brand-gold-bright">Administración de fincas</p>
    ...
    <section class="bg-brand-cream py-16">
        <div class="mx-auto grid max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:px-8">
            <div>
                <h2 class="font-gestico-serif text-3xl font-semibold text-brand-ink">Gestión ordinaria y extraordinaria</h2>
                <p class="mt-5 leading-8 text-brand-text-muted">Nos encargamos del ciclo completo de administración: preparación de juntas, ejecución de acuerdos, atención a consultas, control de proveedores, reclamación de cuotas y seguimiento de incidencias.</p>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                @foreach (['Juntas y actas', 'Mantenimiento', 'Seguros y siniestros', 'Morosidad', 'Contratos', 'Obras y derramas'] as $item)
                    <div class="rounded-md border border-brand-border bg-white p-5 font-gestico-serif text-xl font-semibold text-brand-text-strong">{{ $item }}</div>
                @endforeach
            </div>
        </div>
    </section>
