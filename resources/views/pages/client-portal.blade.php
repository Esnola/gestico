<x-layouts.public title="Área clientes">
    <section class="bg-brand-ink-panel py-16 text-white sm:py-20">
        <div class="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
            <p class="text-sm font-normal uppercase tracking-[0.04em] text-brand-gold-bright">Área clientes</p>
            ...
            <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                <a href="{{ route('login') }}" class="inline-flex justify-center rounded-md bg-brand-gold-bright px-6 py-3 text-sm font-normal text-brand-ink transition hover:bg-brand-gold-soft">Ir al acceso</a>
        ...
        <div class="mx-auto grid max-w-7xl gap-4 px-5 sm:px-6 md:grid-cols-2 lg:px-8">
            @foreach (['Documentación comunitaria', 'Estados de cuentas', 'Facturas y presupuestos', 'Avisos y comunicaciones'] as $item)
                <div class="rounded-lg border border-brand-border bg-white p-6">
                    <h2 class="font-gestico-serif text-2xl font-semibold text-brand-ink-soft">{{ $item }}</h2>
                    <p class="mt-3 leading-7 text-brand-text-card">Módulo previsto para la siguiente fase del proyecto, con permisos y acceso privado por cliente.</p>
                </div>
            @endforeach
        </div>
    </section>
