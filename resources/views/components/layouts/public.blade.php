...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    ...
    <body class="bg-[#f7f4ed] text-[#1e2a2c] antialiased">
        <div class="min-h-screen overflow-hidden">
            <header class="sticky top-0 z-50 border-b border-[#d8d0bd]/70 bg-[#f7f4ed]/92 backdrop-blur">
                <div class="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-3 sm:px-6 lg:px-8">
                    ...
                    <nav class="hidden items-center gap-7 text-sm font-semibold text-[#415154] lg:flex">
                    ...
                    <div class="flex items-center gap-2">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="hidden rounded-md border border-[#b88c16]/35 px-4 py-2 text-sm font-semibold text-[#75560a] transition hover:border-[#b88c16] hover:bg-white sm:inline-flex">Panel</a>
                        @else
                            <a href="{{ route('login') }}" class="hidden rounded-md border border-[#b88c16]/35 px-4 py-2 text-sm font-semibold text-[#75560a] transition hover:border-[#b88c16] hover:bg-white sm:inline-flex">Acceso clientes</a>
                        @endauth
                        <a href="{{ route('contact') }}" class="rounded-md bg-[#a98211] px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#8d6b0d]">Solicitar propuesta</a>
                    </div>
                </div>
                <nav class="flex gap-4 overflow-x-auto border-t border-[#d8d0bd]/70 px-5 py-3 text-sm font-semibold text-[#415154] sm:px-6 lg:hidden">
            ...
            <footer class="border-t border-[#d8d0bd] bg-[#213639] text-white">
                <div class="mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-6 lg:grid-cols-[1.3fr_1fr_1fr] lg:px-8">
                    ...
                    <div>
                        <p class="text-sm font-bold uppercase text-[#d4a824]">Web</p>
                    ...
                    <div>
                        <p class="text-sm font-bold uppercase text-[#d4a824]">Contacto</p>
                        <div class="mt-4 grid gap-2 text-sm text-[#d9e3e4]">
