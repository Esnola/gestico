<x-layouts.public title="Gestión integral de comunidades">
    <section class="relative bg-[#eef1ec]">
        <div class="mx-auto grid min-h-[calc(100vh-82px)] max-w-7xl items-center gap-10 px-5 py-14 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-8">
            <div class="max-w-3xl">
                <p class="text-sm font-bold uppercase text-[#a98211]">Administración de fincas y asesoría para comunidades</p>
                <h1 class="mt-5 text-4xl font-black leading-tight text-[#1f3336] sm:text-5xl lg:text-6xl">Tu comunidad, gestionada con cuentas claras y respuesta profesional.</h1>
                ...
                <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                    <a href="{{ route('contact') }}" class="inline-flex items-center justify-center rounded-md bg-[#a98211] px-6 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#8d6b0d]">Solicitar una propuesta</a>
                    <a href="{{ route('services') }}" class="inline-flex items-center justify-center rounded-md border border-[#b7aa8e] bg-white/70 px-6 py-3 text-sm font-bold text-[#294044] transition hover:border-[#a98211]">Ver servicios</a>
                </div>
                <div class="mt-10 grid max-w-2xl grid-cols-3 divide-x divide-[#d5ccb7] rounded-md border border-[#d5ccb7] bg-white/70">
                    <div class="p-4">
                        <p class="text-2xl font-black text-[#a98211]">24h</p>
                        <p class="mt-1 text-xs font-semibold text-[#58696c]">Información preparada para consulta online</p>
                    </div>
                    <div class="p-4">
                        <p class="text-2xl font-black text-[#a98211]">360°</p>
                        <p class="mt-1 text-xs font-semibold text-[#58696c]">Gestión administrativa, fiscal, laboral y contable</p>
                    </div>
                    <div class="p-4">
                        <p class="text-2xl font-black text-[#a98211]">1</p>
                        <p class="mt-1 text-xs font-semibold text-[#58696c]">Interlocutor coordinando incidencias y acuerdos</p>
                    </div>
                </div>
            </div>
            <div class="relative">
                <div class="rounded-lg border border-[#d8d0bd] bg-white p-5 shadow-xl shadow-[#43595d]/10">
                    <div class="flex items-center justify-between border-b border-[#e7dfcf] pb-4">
                        <div>
                            <p class="text-xs font-bold uppercase text-[#a98211]">Estado de comunidad</p>
                            <p class="mt-1 text-xl font-black text-[#24383b]">Junio 2026</p>
                        </div>
                        <span class="rounded-md bg-[#e8f0ef] px-3 py-2 text-xs font-bold text-[#35575b]">Al día</span>
                    </div>
                    <div class="mt-5 grid gap-3">
                        @foreach (['Presupuesto anual revisado', 'Remesas y cobros controlados', 'Obligaciones fiscales calendarizadas', 'Incidencias con seguimiento'] as $item)
                            <div class="flex items-center justify-between rounded-md border border-[#eee7da] bg-[#faf8f1] p-4">
                                <span class="text-sm font-semibold text-[#405154]">{{ $item }}</span>
                                <span class="text-sm font-black text-[#a98211]">✓</span>
                            </div>
                        @endforeach
                    </div>
                    <div class="mt-5 rounded-md bg-[#24383b] p-5 text-white">
                        <p class="text-sm font-semibold text-[#d7e5e4]">Próxima prioridad</p>
                        <p class="mt-2 text-2xl font-black">Reducir trabajo manual de la junta</p>
    ...
    <section class="bg-white py-20">
        <div class="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
            <div class="max-w-2xl">
                <p class="text-sm font-bold uppercase text-[#a98211]">Qué resolvemos</p>
                <h2 class="mt-3 text-3xl font-black text-[#1f3336] sm:text-4xl">Una administración pensada para comunidades que necesitan orden, transparencia y continuidad.</h2>
            </div>
            <div class="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                @foreach ([
                    ...
                    <article class="rounded-lg border border-[#e4dccd] bg-[#fbfaf6] p-6">
                        <h3 class="text-lg font-black text-[#253b3e]">{{ $title }}</h3>
    ...
    <section class="bg-[#213639] py-20 text-white">
        <div class="mx-auto grid max-w-7xl gap-10 px-5 sm:px-6 lg:grid-cols-[.8fr_1.2fr] lg:px-8">
            <div>
                <p class="text-sm font-bold uppercase text-[#d4a824]">Método</p>
                <h2 class="mt-3 text-3xl font-black sm:text-4xl">Primero entendemos la comunidad. Después ponemos rutina y control.</h2>
            </div>
            <div class="grid gap-4 sm:grid-cols-3">
                @foreach ([
                    ...
                    <div class="rounded-lg border border-white/15 bg-white/6 p-5">
                        <p class="text-sm font-black text-[#d4a824]">{{ $step }}</p>
    ...
    <section class="bg-[#f7f4ed] py-20">
        <div class="mx-auto grid max-w-7xl items-center gap-10 px-5 sm:px-6 lg:grid-cols-[1fr_.9fr] lg:px-8">
            <div>
                <p class="text-sm font-bold uppercase text-[#a98211]">Área de clientes</p>
                <h2 class="mt-3 text-3xl font-black text-[#1f3336] sm:text-4xl">La información privada llegará en la siguiente fase.</h2>
                ...
                <a href="{{ route('client-portal') }}" class="mt-7 inline-flex rounded-md border border-[#b7aa8e] bg-white px-5 py-3 text-sm font-bold text-[#294044] transition hover:border-[#a98211]">Ver planteamiento del portal</a>
            </div>
            <div class="rounded-lg border border-[#d8d0bd] bg-white p-6">
                <div class="grid gap-3">
                    @foreach (['Contratos y documentación comunitaria', 'Movimientos bancarios e informes', 'Facturas, presupuestos y cierres', 'Comunicaciones y avisos importantes'] as $item)
                        <div class="rounded-md bg-[#f4f1e8] px-4 py-3 text-sm font-bold text-[#3f5356]">{{ $item }}</div>
                    @endforeach
                </div>
            </div>
        </div>
